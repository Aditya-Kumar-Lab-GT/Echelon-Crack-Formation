...
sts, scs = 27*0.5, 77	#Tensile strength and compressive strength 77

lch = 3*Gc*E/8/(sts**2) #take lower, about 0.09375 and 0.114 
eps = 1.0 #epsilon should not be chosen to be too large compared to lch. Typically eps<4*lch should work
h = eps/4
...
mesh=Mesh("3dcrack_knauss_extend.xml")
# mesh=refine(mesh)
# mesh=refine(mesh)
n=FacetNormal(mesh)
...
outer= CompiledSubDomain("x[0]<.5*CrackZ", CrackZ=CrackZ)
crack = CompiledSubDomain("abs(x[1]-.5*Ly)<2*h && x[0]<CrackZ+2*h && x[0]>CrackZ-2*h", Ly=Ly, CrackZ=CrackZ, h=h)
lefttop= CompiledSubDomain("x[0]<1e-4 && x[1]>0.5*Ly", Ly=Ly)
leftbottom= CompiledSubDomain("x[0]<1e-4 && x[1]<0.5*Ly", Ly=Ly)
...
disp=0.15 #1% strain rate
r = Expression("t*disp",degree=1,t=0,disp=disp)
r2 = Expression("-t*disp",degree=1,t=0,disp=disp)
ry = Expression("t*disp*0.0",degree=1,t=0,disp=disp)
r0= Expression("(t-tau)*disp",degree=1,t=0,disp=disp, tau=0)
r20= Expression("-(t-tau)*disp",degree=1,t=0,disp=disp, tau=0)

# Define Dirichlet boundary conditions
c=Expression(("0.0","0.0","0.0"),degree=1,t=0)
...
bclt= DirichletBC(V.sub(2), r , lefttop)
bclb= DirichletBC(V.sub(2), r2 , leftbottom)
bcs = [bcr, bclt, bclb]

bclt_du0= DirichletBC(V.sub(2), r0, lefttop)
bclt_du= DirichletBC(V.sub(2), Constant(0.0), lefttop)
bclb_du0= DirichletBC(V.sub(2), r20, leftbottom)
bclb_du= DirichletBC(V.sub(2), Constant(0.0), leftbottom)
bcs_du0 = [bcr, bclt_du0, bclb_du0]
bcs_du = [bcr, bclt_du, bclb_du]
...
while t-stepsize < T:
	...
	r2.t=t; r20.t=t; r20.tau=tau;
	
	stag_iter=1
	rnorm_stag=1
	while stag_iter<15 and rnorm_stag > 1e-7:
